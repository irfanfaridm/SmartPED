...
<div class="max-w-xl mx-auto mt-8 bg-white p-6 rounded shadow">
    ...
    @endif
    <form method="POST" action="{{ route('indihome.store') }}" enctype="multipart/form-data">
        @csrf
        <div class="mb-4">
        ...
        <div class="mb-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
                    ...
                    <div class="mt-3">
                        <input type="hidden" id="selectedDate" name="selected_date" value="{{ now()->format('Y-m-d') }}">
                ...
                <div class="bg-gradient-to-r from-red-50 to-red-100 border border-red-200 rounded-lg p-4">
                    <div class="flex items-center gap-3">
                        <div class="bg-red-600 text-white rounded-lg p-3 text-center min-w-[70px] shadow-lg">
                            <div class="text-xs font-bold uppercase" id="displayMonth">{{ now()->format('M') }}</div>
                            <div class="text-2xl font-bold" id="displayDay">{{ now()->format('d') }}</div>
                            <div class="text-xs" id="displayYear">{{ now()->format('Y') }}</div>
                        </div>
                    ...
                    <div class="mt-3 text-right">
                        <div class="text-xs text-gray-500" id="displayDayOfYear">Hari ke-{{ now()->format('z') + 1 }}</div>
                        <div class="text-xs text-gray-500">dalam tahun {{ now()->format('Y') }}</div>
                    </div>
                </div>
            </div>
        </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    toggleBtn.addEventListener('click', function() {
        if (isNewLocation) {
            ...
            toggleBtn.className = 'px-3 py-2 bg-red-200 text-red-700 rounded hover:bg-red-300 text-sm';
            isNewLocation = true;
        }
    });
